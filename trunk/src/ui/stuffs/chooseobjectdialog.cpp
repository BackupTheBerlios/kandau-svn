	...
	KDialog(parent)
{
	...
	m_listView->fill();
	if ( currentObject ) {
		Q3ListViewItem *item = m_listView->findItem( oidToString( currentObject->oid() ), 0 );
		if ( item )
			m_listView->setSelected( item, true );
	}

	connect( m_listView, SIGNAL(doubleClicked(Q3ListViewItem*,const QPoint&,int)), SLOT(slotDoubleClicked(Q3ListViewItem*,const QPoint&,int)) );
}

Object* ChooseObjectDialog::selectedObject() const
{
	Q3ListViewItem *selected = m_listView->selectedItem();
	if ( ! selected )
		return 0;
...
void ChooseObjectDialog::slotDoubleClicked( Q3ListViewItem*, const QPoint&, int )
{
	slotOk();
}
