...
//ManagerCollectionIterator::data() -> ManagerCollectionIterator::value()
...
Manager::~Manager()
{
	...
	for ( ; it != end; ++it )
		// delete it.value().object(); // DEPRECATED Qt3 -> Qt4 by Percy
        delete it.value().object();

	ManagerCollectionIterator it2( m_collections.begin() );
	ManagerCollectionIterator end2( m_collections.end() );
	for ( ; it2 != end2; ++it2 )
		delete it2.value().collection();
	assert( m_dbBackend );
	delete m_dbBackend;
	m_self = 0;
}
...
void Manager::setMaxObjects( qulonglong max )
{
	m_maxObjects = max;
}

qulonglong Manager::maxObjects() const
{
	return m_maxObjects;
}
...
bool Manager::load( Collection* collection, const QString& query )
{
	...
	ManagerObjectIterator it( m_objects.begin() );
	ManagerObjectIterator end( m_objects.end() );
	QString className = query.toLower();
	Object *obj;
	for ( ; it != end; ++it ) {
		obj = it.value().object();
		if ( obj->classInfo()->name().toLower() == className ) {
	...
	//Q3ValueListConstIterator<OidType> rit( m_removedOids.constBegin() ); // TODO Qt3 -> Qt4 by Percy
	//Q3ValueListConstIterator<OidType> rend( m_removedOids.constEnd() ); // TODO Qt3 -> Qt4 by Percy
    //QList<OidType>::iterator rit( m_removedOids.constBegin() );
    //QList<OidType>::iterator rend( m_removedOids.constEnd() );
// 	for ( ; rit != rend; ++rit ) {
...
// 	}
    ...
    for ( ; m_removedOids.constBegin() != m_removedOids.constEnd(); ++m_removedOids.constBegin() ) {
      if ( collection->contains( *m_removedOids.constBegin() ) ) {
          collection->simpleRemove( *m_removedOids.constBegin() );
      }
  }
...
bool Manager::rollback()
{
	...
	Q3ValueList<OidType> olist;
	for ( ; oit != oend; ++oit ) {
		oit.value().setValid( false );
		if ( oit.value().object()->isModified() )
			olist.append( oit.key() );
	}
	Q3ValueListIterator<OidType> olit( olist.begin() );
	Q3ValueListIterator<OidType> olend( olist.end() );
	for ( ; olit != olend; ++olit ) {
	...
	Q3ValueList<Reference> rlist;
	for ( ; rit != rend; ++rit ) {
		rit.value().setValid( false );
		if ( rit.value().isModified() )
			rlist.append( rit.key() );
	}
	Q3ValueListIterator<Reference> rlit( rlist.begin() );
	Q3ValueListIterator<Reference> rlend( rlist.end() );
	for ( ; rlit != rlend; ++rlit ) {
	...
	for ( ; cit != cend; ++cit ) {
		cit.value().setValid( false );
		if ( cit.value().collection()->modified() )
...
// TODO: Should we put this, or a similar condition?
	...
	Q3ValueVector<OidType>::const_iterator vIt( list.begin() );
	Q3ValueVector<OidType>::const_iterator vEnd( list.end() );
	for ( ; vIt != vEnd; ++vIt ) {
...
void Manager::ensureUnderMaxRelations()
{
	...
	Q3ValueVector<Reference>::const_iterator vIt( list.constBegin() );
	Q3ValueVector<Reference>::const_iterator vEnd( list.constEnd() );
	for ( ; vIt != vEnd; ++vIt ) {
