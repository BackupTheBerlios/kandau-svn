...
{
	assert( object );
	QDomElement root = doc->createElement( object->metaObject()->className() ); // NOTE ... Percy
...
{
    // TODO Qt3 -> Qt4 must be change by Percy
    /*
	...
	for ( ; it != end; ++it )
		objectToElement( it.value().object(), &doc, &root ); // NOTE Percy data X value
