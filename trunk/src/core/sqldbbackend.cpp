...
const ClassInfo* SqlDbBackend::findObject( const OidType& oid, const ClassInfo *classInfo, Q3SqlCursor& cursor )
{
...
bool SqlDbBackend::load( const OidType& oid, Object *object )
{
	...
	object->setOid( 0 );
	Q3SqlCursor cursor( object->classInfo()->name() );
	cursor.select( "to_number( " + oidFieldName() + ", '9999999999G0') = " + oidToString( oid ) );
...
bool SqlDbBackend::load( const Q3SqlCursor &cursor, Object *object )
{
	assert( object );
	Property prop;
	...
	for ( ; pIt != pEnd; ++pIt ) {
		...
		switch ( pIt.data().type() ) {
			case QVariant::Pixmap:
				object->setProperty( pIt.data().name().toAscii(), QVariant(QPixmap( cursor.value( pIt.data().name() ).toByteArray() )) );
				break;
			case QVariant::DateTime:
				object->setProperty( pIt.data().name().toAscii(), cursor.value( pIt.data().name() ).toDateTime() );
				break;
			case QVariant::Date:
				object->setProperty( pIt.data().name().toAscii(), cursor.value( pIt.data().name() ).toDate() );
				break;
			case QVariant::Time:
				object->setProperty( pIt.data().name().toAscii(), cursor.value( pIt.data().name() ).toTime() );
				break;
			case QVariant::ByteArray:
				object->setProperty( pIt.data().name().toAscii(), cursor.value( pIt.data().name() ).toByteArray() );
				break;
			default:
				object->setProperty( pIt.data().name().toAscii(), cursor.value( pIt.data().name() ) );
		}
	}
...
bool SqlDbBackend::load( OidType* relatedOid, const OidType& oid, const RelationInfo* relationInfo )
{
	...
	Q3SqlCursor cursor( searchTable );
...
bool SqlDbBackend::save( Object *object )
{
	...
	for ( ; pIt != pEnd; ++pIt ) {
		...
		if ( prop.type() == QVariant::Pixmap ) {
			...
			//QBuffer buffer( img );
            QBuffer buffer;
            buffer.setData( img ); // NOTE added by Percy ... Qt3- > Qt4
			buffer.open( QIODevice::WriteOnly );
            // DEPRECATED Qt3- > Qt4 by Percy
			//prop.value().toPixmap().save( &buffer, "PNG" );
            prop.value().value<QPixmap>().save( &buffer, "PNG" );
...
bool SqlDbBackend::save( Collection *collection )
{
	...
	Q3SqlCursor cursor( collection->collectionInfo()->name() );
	QSqlRecord *record;
...
bool SqlDbBackend::remove( Object *object )
{
	...
	QSqlQuery query;
	query.prepare( "DELETE FROM " + object->classInfo()->name().toLower() + " WHERE to_number( " + oidFieldName() + ", '9999999999G0') = " + oidToString( object->oid() ) );
	query.exec();
...
bool SqlDbBackend::remove( Collection *collection, const OidType& oid )
{
	...
	QSqlQuery query;
	if ( ! collection->collectionInfo()->isNToOne() ) {
		query.prepare( "DELETE FROM " + collection->collectionInfo()->name().toLower() + " WHERE to_number( " + filterFieldName( collection->collectionInfo() ) + ",'9999999999G0') = " + QString::number( filterValue( collection ) ) + " AND to_number(" + idFieldName( collection->collectionInfo() ) + ",'9999999999G0') = " + oidToString( oid ) );
		query.exec();
	} else {
		query.prepare( "UPDATE " + collection->collectionInfo()->name().toLower() + " SET " + filterFieldName( collection->collectionInfo() ) + " = NULL WHERE to_number(" + idFieldName( collection->collectionInfo() ) + ",'9999999999G0') = " + oidToString( oid ) );
		query.exec();
	}
...
bool SqlDbBackend::createSchema()
{
	...
	for ( ; it != end; ++it ) {
		...
		exec = "CREATE TABLE " +  currentClass->name().toLower() + " ( " + oidFieldName() + " BIGINT PRIMARY KEY, " + sequenceFieldName() + " BIGINT NOT NULL, ";
		...
		for ( ; cIt != cEnd; ++cIt ) {
			...
			for ( ; colIt != colEnd; ++colIt ) {
				rCol = *colIt;
				if ( rCol->childrenClassInfo()->name() == currentClass->name() && rCol->isNToOne() && constraints.filter( rCol->name() ).count() == 0 ) {
		...
		for ( ; colIt != colEnd; ++colIt ) {
			col = *colIt;
			if ( ! tables.filter( col->name() ).count() > 0 && ! col->isNToOne() ) {
	...
	*/
	...
	for ( i = 0; i < tables.count(); ++i ) {
		...
		exec = "CREATE TABLE " + list[ 0 ].toLower() + " ( " + list[ 1 ].toLower() + " BIGINT NOT NULL ";
		if ( Classes::classInfo( list[ 1 ] )->children().count() == 0 )
			exec += " REFERENCES " + list[ 1 ].toLower() + " DEFERRABLE INITIALLY DEFERRED";
		exec += ", " + list[ 2 ].toLower() + " BIGINT NOT NULL ";
		if ( Classes::classInfo( list[2] )->children().count() == 0 )
			exec += " REFERENCES " + list[ 2 ].toLower() + " DEFERRABLE INITIALLY DEFERRED";
		exec += ", " + sequenceFieldName() + " BIGINT NOT NULL , PRIMARY KEY( " + list[1].toLower() + " , " + list[2].toLower() + " ) );";
	...
	for ( i = 0; i < constraints.count(); ++i ) {
		...
		if ( Classes::classInfo( list[ 2 ] )->children().count() == 0 )
			exec = "ALTER TABLE " + list[ 0 ].toLower() + " ADD FOREIGN KEY (" + list[ 1 ].toLower() + ") REFERENCES " + list[ 2 ].toLower() + "( " + oidFieldName() + " ) DEFERRABLE INITIALLY DEFERRED";
		//else
		//	exec = "CREATE DDL!!!";
		m_db->exec( exec );
		if ( m_db->lastError().type() != QSqlError::NoError ) {
...
{
	Q3SqlSelectCursor cursor( "SELECT " + sequenceFieldName() + " FROM " + object->classInfo()->name() + " WHERE to_number( " + oidFieldName() + ", '9999999999G0') = " + oidToString( object->oid() ) );
...
bool SqlDbBackend::commit()
{
	...
	Q3ValueVector< QPair<QString, OidType> >::const_iterator rit( m_removedObjects.begin() );
	Q3ValueVector< QPair<QString, OidType> >::const_iterator rend( m_removedObjects.end() );
	for ( ; rit != rend; ++rit ) {
...
void SqlDbBackend::commitRelations()
{
	...
	for ( ; it != end; ++it ) {
		if ( it.value().isModified() ) {
			kdDebug() << "Relation Oid: " << oidToString( it.value().oid() ) << endl;
			save( it.key().oid(), it.value().relationInfo(), it.value().oid() );
		}
	}
}
...
QString SqlDbBackend::expandDotsString( const QString& string )
{
    ...
    // NOTE added by Percy
	MTokenizer tokenizer( string, QStringList() << "." );
...
bool SqlDbBackend::load( Collection* collection, const QString& query )
{
	...
	Q3SqlSelectCursor cursor( newQuery );
	cursor.select();
	while ( cursor.next() ) {
